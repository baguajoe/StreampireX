...
@beat_store_bp.route('/api/beats/producers', methods=['GET'])
def browse_producers():
    try:
        search = request.args.get('q', '').strip()
        genre = request.args.get('genre', '').strip()
        sort = request.args.get('sort', 'popular')

        producers_query = db.session.query(
            User.id, User.username, User.profile_image, User.bio,
            db.func.count(Beat.id).label('beat_count'),
            db.func.coalesce(db.func.sum(Beat.plays), 0).label('total_plays'),
            db.func.coalesce(db.func.sum(Beat.sales_count), 0).label('total_sales'),
            db.func.min(Beat.base_price).label('min_price'),
            db.func.max(Beat.base_price).label('max_price'),
        ).join(Beat, Beat.producer_id == User.id).filter(
            Beat.is_active == True, Beat.is_sold_exclusive == False
        ).group_by(User.id)

        if search:
            producers_query = producers_query.filter(
                db.or_(User.username.ilike(f"%{search}%"), Beat.genre.ilike(f"%{search}%"))
            )
        if genre:
            producers_query = producers_query.filter(Beat.genre.ilike(f"%{genre}%"))

        if sort == 'most_beats':
            producers_query = producers_query.order_by(db.desc('beat_count'))
        elif sort == 'newest':
            producers_query = producers_query.order_by(User.id.desc())
        elif sort == 'best_selling':
            producers_query = producers_query.order_by(db.desc('total_sales'))
        else:
            producers_query = producers_query.order_by(db.desc('total_plays'))

        results = producers_query.limit(50).all()
        producers = []
        for r in results:
            top_genres = db.session.query(Beat.genre, db.func.count(Beat.id)).filter(
                Beat.producer_id == r.id, Beat.is_active == True, Beat.genre.isnot(None)
            ).group_by(Beat.genre).order_by(db.func.count(Beat.id).desc()).limit(3).all()
            producers.append({
                "id": r.id, "username": r.username,
                "profile_image": r.profile_image, "bio": r.bio,
                "beat_count": r.beat_count, "total_plays": r.total_plays,
                "total_sales": r.total_sales,
                "price_range": f"${r.min_price:.0f}-${r.max_price:.0f}" if r.min_price else "N/A",
...
@beat_store_bp.route('/api/beats/producer/<int:producer_id>', methods=['GET'])
def get_producer_profile(producer_id):
    user = User.query.get(producer_id)
    if not user:
        return jsonify({"error": "Producer not found"}), 404
    beats = Beat.query.filter_by(producer_id=producer_id, is_active=True).order_by(Beat.created_at.desc()).all()
    stats = db.session.query(
        db.func.count(Beat.id),
        db.func.coalesce(db.func.sum(Beat.plays), 0),
        db.func.coalesce(db.func.sum(Beat.sales_count), 0),
    ).filter(Beat.producer_id == producer_id, Beat.is_active == True).first()
    return jsonify({
        "producer": {
            "id": user.id, "username": user.username,
            "profile_image": user.profile_image, "bio": user.bio,
            "beat_count": stats[0] if stats else 0,
            "total_plays": stats[1] if stats else 0,
            "total_sales": stats[2] if stats else 0,
        },
        "beats": [b.serialize_card() for b in beats]
    }), 200
