# =============================================================================
# VIDEO_TIERS.py - 3-Tier Configuration (Free, Basic, Premium)
# =============================================================================
# Uses tier names: free, basic, premium
# Premium is now the top tier with enhanced limits
# =============================================================================
...
VIDEO_EDITOR_TIERS = {
    ...
    'premium': {
        ...
        # Export Limits (BOOSTED - now top tier)
        'max_export_quality': '4k',
...
STREAMING_TIERS = {
    ...
    'premium': {
        'name': 'Premium',
        'enabled': True,
        'max_quality': '4k',  # BOOSTED from 1080p
        'max_duration_hours': 12,  # BOOSTED from 8
        'max_bitrate_kbps': 15000,  # BOOSTED
        'vod_storage_gb': 100,  # BOOSTED
        'vod_retention_days': 90,  # BOOSTED
        'simulcast': True,
        'simulcast_destinations': 5,  # BOOSTED
        'stream_overlay': True,
        'chat_enabled': True,
        'monetization': True,
        'tips_enabled': True,
...
CLIPS_TIERS = {
    ...
    'premium': {
        'name': 'Premium',
        'clips_per_day': None,  # Unlimited (BOOSTED)
        'max_duration_seconds': 600,  # 10 minutes (BOOSTED)
        'max_file_size_mb': 2048,  # 2GB (BOOSTED)
        'effects': True,
        'filters': True,
        'music_library': True,
        'premium_music': True,
...
def can_export_4k(user_tier):
    ...
    return tier.get('max_export_quality') == '4k'
...
def get_max_stream_duration(user_tier):
    """Get max stream duration in hours"""
    tier = get_streaming_tier(user_tier)
...
def get_max_collaborators(user_tier):
    """Get max number of collaborators"""
    tier = get_video_editor_tier(user_tier)
...
def validate_upload(user_tier, file_size_bytes, duration_minutes=None):
    ...
    max_size_bytes = tier.get('max_file_size_gb', 2) * (1024 ** 3)
    if file_size_bytes > max_size_bytes:
        return False, f'File exceeds your {tier.get("max_file_size_gb")}GB limit. Upgrade for larger uploads.'
...
def validate_stream_start(user_tier):
    ...
    if not tier.get('enabled'):
        return False, 'Live streaming requires Basic plan or higher.', None
