...
def check_and_reset_monthly_credits(credit, user_id):
    """Check if monthly credits need to be reset and do so"""
    now = datetime.utcnow()
    
    # If never reset, or last reset was more than 30 days ago
    needs_reset = False
    ...
    if needs_reset:
        ...
        if free_credits > 0:
            credit.reset_monthly(free_credits)
            db.session.commit()
...
def purchase_credit_pack():
    ...
    try:
        ...
        # Check tier — free users can't buy credits
        tier = get_user_tier(user_id)
        if tier == 'free':
            return jsonify({
...
def handle_credit_pack_payment(session):
    ...
    try:
        ...
        current_app.logger.info(
            f"✅ Credits added: {credits_amount} credits to user {user_id} "
            f"(pack: {pack_id})"
        )
        
    except Exception as e:
        current_app.logger.error(f"❌ Error handling credit payment: {e}")
        db.session.rollback()
        raise
...
# =============================================================================
# DEDUCT CREDITS (Internal helper — used by video generation routes)
# =============================================================================
