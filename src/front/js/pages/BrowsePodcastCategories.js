...
import "../../styles/BrowsePodcasts.css"; // dark theme shared with radio-style
...
const BrowsePodcastCategories = () => {
  ...
  // Map API category codes -> display names
    ...
    try {
      ...
      const validPodcasts = podcasts
        ...
        .map((podcast) => ({
          ...
          description:
            podcast.description || "A great podcast on StreampireX",
  ...
  // 🌐 External podcast fallback (placeholder data)
  ...
  useEffect(() => {
    const loadAllPodcasts = async () => {
      ...
      try {
        ...
        setError(
          "Unable to load community podcasts. Showing featured content only."
        );
  ...
  const allAvailablePodcasts = React.useMemo(
    () => [...seedPodcasts, ...userPodcasts, ...externalPodcasts],
  ...
  const filteredPodcasts = React.useMemo(() => {
    ...
    return allAvailablePodcasts.filter(
      (podcast) => podcast.category === selectedCategory
    );
  }, [selectedCategory, allAvailablePodcasts]);

  // Grouped by category (used for “all sections” view)
            ...
            {podcastList.map((podcast) => (
              ...
              >
                ...
                >
                  ...
                  <p>
                    {podcast.episode_count} episodes • {podcast.duration}
                  </p>
                  {podcast.plays > 0 && (
        ...
        <>
          {selectedCategory ? (
            ...
            <>
              ...
              {categories.map((category) => {
                const categoryPodcasts = userPodcasts.filter(
              ...
              {userPodcasts.length === 0 &&
                ...
                !loading && (
                  ...
                  >
                    ...
                    <p>
                      StreampireX is the perfect platform for podcasters. Create
                      your show and reach new audiences!
                    </p>
                    <button
                      onClick={() => navigate("/create-podcast")}
                      style={{
          ...
          >
            <p>
              ...
              {seedPodcasts.length} originals • {userPodcasts.length} community •{" "}
