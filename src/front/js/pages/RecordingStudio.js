      ...
      <div className="daw-main">
            ...
            <div className="rs-split-bottom" style={{height:`${100-splitTopH}%`}}>
              ...
              <div className="daw-console"><div className="daw-console-scroll">
                {tracks.map((t,i)=>{
                  ...
                  return(
                    <div key={t.id??i} className={`daw-channel${i===selectedTrack?" selected":""}`} onClick={()=>setSelectedTrack(i)}>
                      ...
                      <div className="daw-ch-fader-area"><div className="daw-ch-fader-row">
                        <div className="daw-ch-fader">
                          <input type="range" min={0} max={1} step={0.01} value={t.volume??0.8} orient="vertical" onChange={e=>updateTrack(i,{volume:parseFloat(e.target.value)})}/>
                        </div>
                ...
                <div className="daw-channel master-channel">
                  <div className="daw-ch-controls">
                    <button className="daw-ch-btn">M</button><button className="daw-ch-btn">S</button>
                  </div>
                  <div className="daw-ch-fader-area"><div className="daw-ch-fader-row">
        ...
        {viewMode==="pianoroll"&&(
          <div className="daw-pianoroll-view">
            <PianoRoll notes={pianoRollNotes} onNotesChange={handlePianoRollNotesChange} bpm={bpm} timeSignature={timeSignature}
              musicalKey={pianoRollKey} scale={pianoRollScale} isPlaying={isPlaying} currentBeat={playheadBeat}
              audioContext={audioCtxRef.current} onExport={handlePianoRollExport} onClose={()=>setViewMode("beatmaker")}
              isEmbedded={true} editingRegion={editingRegion} onSaveToRegion={savePianoRollToRegion}/>
          </div>
        )}
        ...
        {viewMode==="chords"&&(
          <div className="daw-chords-view">
            <ChordProgressionGenerator musicalKey={pianoRollKey} scale={pianoRollScale} bpm={bpm} timeSignature={timeSignature}
              onInsertChords={handleChordInsert} onKeyChange={handleChordKeyChange} audioContext={audioCtxRef.current}
              onClose={()=>setViewMode("pianoroll")} isEmbedded={true}/>
          </div>
        )}
        ...
        {viewMode==="sounds"&&(
          <div className="daw-freesound-view">
            <FreesoundBrowser audioContext={audioCtxRef.current}
              onSoundSelect={(audioBuffer,name,audioUrl)=>{
                const armedMidi=tracks.findIndex(t=>t.armed&&(t.trackType==="midi"||t.trackType==="instrument"));
                if(armedMidi!==-1){instrumentEngine.loadSampleOntoTrack(armedMidi,audioBuffer,name,60);setStatus(`🎵 "${name}" → Track ${armedMidi+1} — play keys to hear`);}
              ...
              onApplyMicProfile={handleApplyMicProfile} onClose={()=>setShowMicSimModal(false)} isEmbedded={true}/>
          </div>
        )}
        ...
        {viewMode==="vocal"&&(
          <div className="daw-vocal-view rs-view-scroll">
            <VocalProcessor audioContext={audioCtxRef.current} liveStream={micSimStream}
              onRecordingComplete={(blob)=>{
                const ai=tracks.findIndex(t=>t.armed); if(ai===-1){setStatus("⚠ Arm a track first");return;}
                const ctx=getCtx(); const audioUrl=URL.createObjectURL(blob);
                blob.arrayBuffer().then(ab=>ctx.decodeAudioData(ab)).then(buf=>{updateTrack(ai,{audioBuffer:buf,audio_url:audioUrl});createRegionFromRecording(ai,buf,audioUrl);uploadTrack(blob,ai);setStatus(`✓ Vocal recorded → Track ${ai+1}`);setViewMode("arrange");}).catch(e=>setStatus(`✗ ${e.message}`));
              }}
        ...
        {viewMode==="aimix"&&(
          <div className="daw-aimix-view rs-view-scroll">
            <AIMixAssistant tracks={tracks} projectId={projectId} bpm={bpm} timeSignature={timeSignature}
              onApplyVolume={handleAIApplyVolume} onApplyPan={handleAIApplyPan} onApplyEQ={handleAIApplyEQ}
              onApplyCompression={handleAIApplyCompression} onClose={()=>setViewMode("arrange")} isEmbedded={true}/>
          </div>
        )}
        ...
        {viewMode==="voicemidi"&&(
          <div className="daw-voicemidi-view">
            <VoiceToMIDI audioContext={audioCtxRef.current} bpm={bpm} isEmbedded={true}
              onNoteOn={({note,velocity})=>{const armedIdx=tracks.findIndex(t=>t.armed&&(t.trackType==="midi"||t.trackType==="instrument"));if(armedIdx!==-1)instrumentEngine.playNoteOnTrack(armedIdx,note,velocity);}}
              onNoteOff={({note})=>{const armedIdx=tracks.findIndex(t=>t.armed&&(t.trackType==="midi"||t.trackType==="instrument"));if(armedIdx!==-1)instrumentEngine.stopNoteOnTrack(armedIdx,note);}}
              onNotesGenerated={(notes)=>{
                if(!notes?.length) return;
                const armedIdx=tracks.findIndex(t=>t.armed&&(t.trackType==="midi"||t.trackType==="instrument"));
                if(armedIdx===-1) return;
                const beatsPerSec=bpm/60;
                const midiNotes=notes.map((n,i)=>({id:`voice_${Date.now()}_${i}`,note:n.note,velocity:n.velocity||80,startBeat:(n.startTime||n.time||0)*beatsPerSec,duration:Math.max((n.duration||0.25)*beatsPerSec,0.125)}));
                const inst=instrumentEngine.getTrackInstrument(armedIdx);
                const region=createMidiRegionFromNotes(midiNotes,inst?.isDrum?"Beatbox Pattern":"Voice Melody");
                if(playheadBeat>0){const offset=region.startBeat;region.startBeat=playheadBeat;region.notes=region.notes.map(n=>({...n,startBeat:n.startBeat-offset}));}
                setTracks(prev=>prev.map((t,i)=>i===armedIdx?{...t,regions:[...(t.regions||[]),region]}:t));
                setStatus(`🎤 ${midiNotes.length} notes from voice → Track ${armedIdx+1}`);
              }}
              onSendToTrack={(audioBuffer,name)=>{const idx=selectedTrackIndex;updateTrack(idx,{audioBuffer,name:name||tracks[idx].name});setViewMode("arrange");setStatus(`Vocal MIDI render → Track ${idx+1}`);}}
        ...
        {viewMode==="takelanes"&&(
          <div className="rs-flex-hidden">
            <TakeLanes audioContext={audioCtxRef.current} bpm={bpm}
              onCompositeReady={(compositeBuffer,name)=>{
                const ai=tracks.findIndex(t=>t.armed); const targetIdx=ai!==-1?ai:selectedTrackIndex;
                const audioUrl=URL.createObjectURL(new Blob([],{type:'audio/wav'}));
                updateTrack(targetIdx,{audioBuffer:compositeBuffer,audio_url:audioUrl,name:name||'Comp'});
                createRegionFromImport(targetIdx,compositeBuffer,name||'Comp',audioUrl);
                setStatus(`✓ ${name} → Track ${targetIdx+1}`); setViewMode('arrange');
              }} isEmbedded={true}/>
          </div>
        )}
            ...
            <div className="rs-analog-scale">
                ...
                <div className="rs-settings-panel">
                  <div className="rs-dark-card">
                    <div className="rs-card-header">
                      <div><h4 className="rs-card-title">📼 Tape Saturation</h4><p className="rs-card-subtitle">Analog warmth via waveshaper + lowpass filter</p></div>
                      <label className="rs-toggle-label">
                        ...
                        <span className={`daw-tape-label${tapeEnabled?" on":""}`}>{tapeEnabled?'ON':'OFF'}</span>
                      </label>
                    </div>
                    {[['DRIVE',tapeDrive,setTapeDrive,'drive'],['WARMTH',tapeWarmth,setTapeWarmth,'warmth']].map(([lbl,val,setter,key])=>(
                      <div key={lbl} className="rs-param-row">
                        <div className="rs-param-header"><span>{lbl}</span><span className="rs-orange">{(val*100).toFixed(0)}%</span></div>
                        <input type="range" min={0} max={1} step={0.01} value={val} className="rs-range-orange" onChange={e=>{const v=parseFloat(e.target.value);setter(v);setFx(f=>({...f,tapeSaturation:{...f.tapeSaturation,[key]:v}}));rebuildTrackGraph&&tracks.forEach(t=>rebuildTrackGraph(t.id));}}/>
                      </div>
                    ))}
                  </div>
                  <div className="rs-dark-card">
                    <div className="rs-card-header">
                      <div><h4 className="rs-card-title">⚡ Harmonic Exciter</h4><p className="rs-card-subtitle">Aphex-style presence enhancer</p></div>
                      <label className="rs-toggle-label">
                        <input type="checkbox" checked={harmonicEnabled} onChange={e=>{setHarmonicEnabled(e.target.checked);setFx(f=>({...f,exciter:{...f.exciter,enabled:e.target.checked}}));tracks.forEach(t=>rebuildTrackGraph&&rebuildTrackGraph(t.id));}}/>
                        <span className={`daw-harmonic-label${harmonicEnabled?" on":""}`}>{harmonicEnabled?'ON':'OFF'}</span>
                      </label>
                    </div>
                    <div className="rs-param-header"><span>AMOUNT</span><span className="rs-yellow-text">{(harmonicAmount*100).toFixed(0)}%</span></div>
                    <input type="range" min={0} max={1} step={0.01} value={harmonicAmount} className="rs-range-yellow" onChange={e=>{const v=parseFloat(e.target.value);setHarmonicAmount(v);setFx(f=>({...f,exciter:{...f.exciter,amount:v}}));tracks.forEach(t=>rebuildTrackGraph&&rebuildTrackGraph(t.id));}}/>
                  </div>
                </div>
              )}
              {analogSubview==='pedals'&&(
                <div className="rs-amp-panel">
                  <h4 className="daw-pedals-heading">🎛️ Signal Chain</h4>
                  <p className="daw-pedals-sub">Analog-modeled effects in series — Tuner → Compressor → Overdrive → Chorus → Delay → Reverb</p>
                  <div className="rs-chain-grid">
                    {[['🎵','Tuner'],['🗜️','Compressor'],['🔥','Overdrive'],['🌊','Chorus'],['⏱️','Delay'],['🏔️','Reverb']].map(([icon,name])=>(
                      <div key={name} className="rs-effect-card daw-pedal-card">
              ...
              {analogSubview==='console'&&(
                <div className="rs-console-panel">
                  <div className="rs-console-label">CONSOLE CHARACTER — Applied to each track and master bus</div>
                  <div className="rs-console-btn-row">
                    {Object.entries(CONSOLE_BOARDS).map(([id,b])=>(
                      <button key={id} onClick={()=>{const nc={};tracks.forEach(t=>{nc[t.id]=id;});setTrackConsoleChar(nc);}} className="daw-console-board-btn" style={{borderColor:b.color,background:id==='none'?'#0d1117':`${b.color}22`,color:b.color}}>{b.name}</button>
                    ))}
                  </div>
                  <div className="rs-master-bus-label">MASTER BUS</div>
                  <div className="rs-console-btn-row">
                    {Object.entries(CONSOLE_BOARDS).map(([id,b])=>(
                      <button key={id} onClick={()=>setMasterConsoleChar(id)} className="daw-console-board-btn-sm" style={{borderColor:masterConsoleChar===id?b.color:'#21262d',background:masterConsoleChar===id?`${b.color}22`:'#0d1117',color:masterConsoleChar===id?b.color:'#4e6a82'}}>{b.name}</button>
                    ))}
                  </div>
                  <div className="rs-console-hint">Per-track: Use the Console tab dropdown on each channel strip</div>
                </div>
              )}
            </div>
          </div>
