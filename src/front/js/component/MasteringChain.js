...
const MasteringChain = ({ audioContext, inputNode, outputNode, onClose, isEmbedded }) => {
  ...
  const [eq,       setEq]       = useState({ enabled: true,  lowShelf: 0, lowFreq: 80, midPeak: 0, midFreq: 2500, midQ: 1.0, highShelf: 0, highFreq: 10000, lowPassEnabled: false, lowPassFreq: 20000 });
  ...
  const [expanded,    setExpanded]    = useState({ eq: true, limiter: true });
  ...
  useEffect(() => {
    ...
    // HPF
    n.hpf1 = ctx.createBiquadFilter(); n.hpf1.type = 'highpass'; n.hpf1.frequency.value = hpf.freq; n.hpf1.Q.value = 0.707;
    n.hpf2 = ctx.createBiquadFilter(); n.hpf2.type = 'highpass'; n.hpf2.frequency.value = hpf.freq; n.hpf2.Q.value = 0.707;

    // EQ
    n.eqLowShelf  = ctx.createBiquadFilter(); n.eqLowShelf.type = 'lowshelf';  n.eqLowShelf.frequency.value = eq.lowFreq;  n.eqLowShelf.gain.value = eq.lowShelf;
    n.eqMidPeak   = ctx.createBiquadFilter(); n.eqMidPeak.type  = 'peaking';   n.eqMidPeak.frequency.value  = eq.midFreq;  n.eqMidPeak.gain.value  = eq.midPeak; n.eqMidPeak.Q.value = eq.midQ;
    n.eqHighShelf = ctx.createBiquadFilter(); n.eqHighShelf.type= 'highshelf'; n.eqHighShelf.frequency.value= eq.highFreq; n.eqHighShelf.gain.value= eq.highShelf;

    // Exciter (harmonic saturation on high shelf)
    n.exciterLP   = ctx.createBiquadFilter(); n.exciterLP.type = 'highpass'; n.exciterLP.frequency.value = exciter.freq;
    n.exciterShape= ctx.createWaveShaper();
    n.exciterGain = ctx.createGain(); n.exciterGain.gain.value = exciter.amount / 100 * 0.5;
    n.exciterMix  = ctx.createGain(); n.exciterMix.gain.value = 1;
    // Soft clip curve for exciter
    const excCurve = new Float32Array(256);
    for (let i = 0; i < 256; i++) { const x = (i / 128) - 1; excCurve[i] = (3 * x / 2) * (1 - (x * x) / 3); }
    n.exciterShape.curve = excCurve;

    // Stereo widener (uses splitter/merger + mid-side)
    n.splitter = ctx.createChannelSplitter(2);
    n.merger   = ctx.createChannelMerger(2);
    n.widthGainL = ctx.createGain(); n.widthGainL.gain.value = widener.width;
    n.widthGainR = ctx.createGain(); n.widthGainR.gain.value = widener.width;

    // Limiter
    n.limiterDrive = ctx.createGain(); n.limiterDrive.gain.value = Math.pow(10, limiter.drive / 20);
    ...
    // Analysers
    n.analyserL = ctx.createAnalyser(); n.analyserL.fftSize = 256; n.analyserL.smoothingTimeConstant = 0.3;
    n.analyserR = ctx.createAnalyser(); n.analyserR.fftSize = 256; n.analyserR.smoothingTimeConstant = 0.3;
    n.analyserFFT= ctx.createAnalyser(); n.analyserFFT.fftSize = 2048; n.analyserFFT.smoothingTimeConstant = 0.7;

    analyserLRef.current = n.analyserL;
    analyserRRef.current = n.analyserR;
    analyserFFT.current  = n.analyserFFT;

    // ── Wire: input → HPF → EQ → exciter → widener → limiter → analysers → output
    try {
      ...
      n.eqHighShelf.connect(n.exciterMix); // dry signal also goes to mix
      ...
      n.limiterComp.connect(n.splitter); // re-split for per-channel analysis
      // Also connect to output
      n.limiterComp.connect(outputNode);
      n.splitter.connect(n.analyserL, 0);
      n.splitter.connect(n.analyserR, 1);
    } catch (e) { console.warn('[MasteringChain] wire error', e); }

    nodesRef.current = n;
    return () => {
      Object.values(n).forEach(node => { try { node.disconnect(); } catch (_) {} });
    };
  ...
  useEffect(() => {
    const n = nodesRef.current; if (!n.limiterComp) return;
    const t = audioContext?.currentTime ?? 0;
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      // Momentary LUFS ≈ 20*log10(rms) - K-weighting offset (~-0.7dBFS)
      const momentary = rms > 0.000001 ? 20 * Math.log10(rms) - 0.7 : -100;

      // Short-term: rolling 3s buffer
      ...
      const shortLUFS = 10 * Math.log10(shortSum / lufsShortBuf.current.length);

      // Integrated: rolling 30s
      ...
      const integrated = 10 * Math.log10(sum / lufsHistory.current.length);

      setLufs(isFinite(integrated) ? integrated : -100);
      setLufsShort(isFinite(shortLUFS) ? shortLUFS : -100);

      // True peak
      const tp = 20 * Math.log10(Math.max(peakL, peakR));
      ...
      // Goniometer dots (L vs R as X/Y)
      const gonioPoints = [];
      const step = Math.max(1, Math.floor(bufL.length / 60));
      for (let i = 0; i < bufL.length; i += step) {
        const m = bufL[i] + bufR[i]; // mid
        const s = bufL[i] - bufR[i]; // side
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <span style={{ color: '#e6edf3', fontWeight: 800, fontSize: '12px', letterSpacing: '0.15em' }}>MASTERING CHAIN</span>
        <div style={{ flex: 1 }} />

        {/* Target selector */}
        <span style={{ color: '#484f58', fontSize: '9px' }}>TARGET</span>
        <select value={target} onChange={e => setTarget(e.target.value)} style={{
          background: '#161b22', border: '1px solid #30363d', color: '#ffd60a',
          borderRadius: '5px', padding: '3px 8px', fontFamily: 'inherit', fontSize: '9px', fontWeight: 800,
        }}>
        ...
        {['chain','meters','spectrum','gonio'].map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)} style={{
            ...
            fontFamily: 'inherit', fontSize: '9px', fontWeight: 800, textTransform: 'uppercase',
          }}>{tab}</button>
        ))}
      ...
      <div style={{
        ...
        padding: '7px 14px', borderBottom: '1px solid #1c2128',
        background: '#0a0e14', flexShrink: 0,
      }}>
        {/* Integrated LUFS gauge */}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
          ...
          {clipping && <span style={{ color: '#ff2d55', fontSize: '9px', fontWeight: 800, animation: 'blink 0.5s infinite' }}>CLIP</span>}
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
          ...
          <div style={{ position: 'relative', width: '180px', height: '8px', background: '#21262d', borderRadius: '4px', overflow: 'hidden' }}>
            <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: `${Math.max(0, Math.min(100, ((lufs + 30) / 30) * 100))}%`, background: lufsColor(lufs), borderRadius: '4px', transition: 'width 0.15s, background 0.15s' }} />
            {/* Target line */}
            <div style={{ position: 'absolute', top: 0, bottom: 0, left: `${((tgt.lufs + 30) / 30) * 100}%`, width: '1px', background: '#fff' }} />
          </div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
          ...
          <div style={{ position: 'relative', width: '60px', height: '8px', background: '#21262d', borderRadius: '4px' }}>
            <div style={{ position: 'absolute', left: `${((correlation + 1) / 2) * 100}%`, top: '-2px', bottom: '-2px', width: '3px', background: correlation > 0 ? '#00ffc8' : '#ff6b6b', borderRadius: '2px', transform: 'translateX(-50%)', transition: 'left 0.1s' }} />
            <div style={{ position: 'absolute', left: '50%', top: 0, bottom: 0, width: '1px', background: '#30363d' }} />
          </div>
          <span style={{ color: correlation > 0 ? '#00ffc8' : '#ff6b6b', fontSize: '9px', fontWeight: 700, minWidth: '28px' }}>{correlation.toFixed(2)}</span>
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            ...
            <ToolSlot id="widener" label="Stereo Widener" icon="⟺" color="#30d158"
              ...
              <div style={{ display: 'flex', gap: '6px', marginTop: '8px' }}>
                {[0, 0.5, 1.0, 1.5, 2.0].map(w => (
                  <button key={w} onClick={() => setWidener(p => ({ ...p, width: w }))} style={{
                    flex: 1, background: widener.width === w ? '#30d15822' : 'none',
                    border: `1px solid ${widener.width === w ? '#30d158' : '#21262d'}`,
                    color: widener.width === w ? '#30d158' : '#484f58',
                    borderRadius: '4px', padding: '3px 0', cursor: 'pointer', fontFamily: 'inherit', fontSize: '8px', fontWeight: 800,
            ...
            <ToolSlot id="limiter" label="True Peak Limiter" icon="⊟" color="#ff6b6b"
              ...
              <div style={{ display: 'flex', gap: '4px', marginBottom: '8px' }}>
                {Object.entries(STREAMING_TARGETS).map(([k, v]) => (
                  <button key={k} onClick={() => { setTarget(k); setLimiter(p => ({ ...p, ceiling: v.tp })); }} style={{
                    background: 'none', border: `1px solid ${target === k ? '#ff6b6b' : '#21262d'}`,
                    color: target === k ? '#ff6b6b' : '#484f58', borderRadius: '4px', padding: '2px 5px',
                    cursor: 'pointer', fontFamily: 'inherit', fontSize: '8px', fontWeight: 700,
                  }}>{k.split(' ')[0]}</button>
                ))}
              </div>
        ...
        {activeTab === 'meters' && (
          <div style={{ flex: 1, display: 'flex', gap: '10px', padding: '12px', overflow: 'hidden' }}>
            {/* LUFS history graph */}
            <div style={{ flex: 2, background: '#0a0e14', border: '1px solid #1c2128', borderRadius: '8px', padding: '10px', display: 'flex', flexDirection: 'column' }}>
              <div style={{ color: '#484f58', fontSize: '9px', letterSpacing: '0.12em', marginBottom: '8px' }}>INTEGRATED LUFS — HISTORY</div>
              <div style={{ flex: 1, display: 'flex', alignItems: 'flex-end', gap: '1px' }}>
                {lufsHistory.current.slice(-120).map((v, i) => {
                  ...
                  return <div key={i} style={{ flex: 1, height: `${h}%`, background: lufsColor(v), borderRadius: '1px 1px 0 0', opacity: 0.7 + 0.3 * (i / 120) }} />;
                })}
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '4px' }}>
                <span style={{ color: '#2d333b', fontSize: '8px' }}>-40 LUFS</span>
                <span style={{ color: '#ffd60a', fontSize: '9px', fontWeight: 800 }}>Target: {tgt.lufs} LUFS</span>
                <span style={{ color: '#2d333b', fontSize: '8px' }}>0 LUFS</span>
              </div>
            </div>

            {/* Per-platform check */}
            <div style={{ flex: 1, background: '#0a0e14', border: '1px solid #1c2128', borderRadius: '8px', padding: '10px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
              <div style={{ color: '#484f58', fontSize: '9px', letterSpacing: '0.12em', marginBottom: '4px' }}>PLATFORM CHECK</div>
              {Object.entries(STREAMING_TARGETS).map(([k, v]) => {
                ...
                return (
                  <div key={k} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <span style={{ color: ok ? '#00ffc8' : loud ? '#ff6b6b' : '#ffd60a', fontSize: '10px' }}>{ok ? '✓' : loud ? '↓' : '↑'}</span>
                    <span style={{ flex: 1, color: '#6e7681', fontSize: '9px' }}>{k}</span>
                    <span style={{ color: '#484f58', fontSize: '8px' }}>{v.lufs} LUFS</span>
                    <span style={{ color: ok ? '#00ffc8' : '#ff6b6b', fontSize: '9px', fontWeight: 700, minWidth: '48px', textAlign: 'right' }}>
        ...
        {activeTab === 'spectrum' && (
          <div style={{ flex: 1, padding: '12px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
            <div style={{ flex: 1, background: '#0a0e14', border: '1px solid #1c2128', borderRadius: '8px', padding: '10px', display: 'flex', flexDirection: 'column' }}>
              <div style={{ color: '#484f58', fontSize: '9px', letterSpacing: '0.12em', marginBottom: '8px' }}>SPECTRAL ANALYZER</div>
              <div style={{ flex: 1, display: 'flex', alignItems: 'flex-end', gap: '2px' }}>
                {spectrumData.map((v, i) => {
                  const hue = 180 - i * 2; // teal → orange gradient
                  return (
                    <div key={i} style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '4px', color: '#2d333b', fontSize: '8px' }}>
                {['20Hz','100Hz','500Hz','1kHz','5kHz','10kHz','20kHz'].map(f => <span key={f}>{f}</span>)}
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'gonio' && (
          <div style={{ flex: 1, padding: '12px', display: 'flex', gap: '12px', overflow: 'hidden' }}>
            <div style={{ flex: 1, background: '#0a0e14', border: '1px solid #1c2128', borderRadius: '8px', padding: '10px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <div style={{ color: '#484f58', fontSize: '9px', letterSpacing: '0.12em', marginBottom: '8px' }}>GONIOMETER (STEREO PHASE)</div>
              <div style={{ position: 'relative', width: '200px', height: '200px', borderRadius: '50%', background: '#0d1117', border: '1px solid #21262d', overflow: 'hidden' }}>
                {/* Grid lines */}
                ...
                {gonioData.slice(-120).map((pt, i) => (
                  <div key={i} style={{
                    position: 'absolute',
                    left: `${50 + pt.x * 0.6}%`, top: `${50 - pt.y * 0.6}%`,
              ...
              <div style={{ marginTop: '10px', color: '#6e7681', fontSize: '9px', textAlign: 'center' }}>
                Correlation: <span style={{ color: correlation > 0.5 ? '#00ffc8' : correlation > 0 ? '#ffd60a' : '#ff6b6b', fontWeight: 800 }}>{correlation.toFixed(3)}</span>
                {correlation < 0 && <span style={{ color: '#ff6b6b', marginLeft: '8px' }}>⚠ PHASE ISSUES</span>}
              </div>
            </div>
          </div>
        )}
...
const ToolSlot = ({ id, label, icon, color, enabled, onToggle, expanded, onExpand, children }) => (
  <div style={{
    marginBottom: '4px', border: `1px solid ${enabled ? `${color}44` : '#1c2128'}`,
    borderRadius: '7px', background: enabled ? `${color}08` : '#0a0e14', overflow: 'hidden',
  }}>
    <div onClick={onExpand} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '7px 10px', cursor: 'pointer' }}>
      <button onClick={e => { e.stopPropagation(); onToggle(); }} style={{
        width: '16px', height: '16px', borderRadius: '50%', flexShrink: 0,
        background: enabled ? color : '#21262d', border: `1px solid ${enabled ? color : '#30363d'}`,
        cursor: 'pointer', padding: 0, boxShadow: enabled ? `0 0 8px ${color}88` : 'none',
      }} />
      <span style={{ fontSize: '13px' }}>{icon}</span>
      <span style={{ color: enabled ? '#e6edf3' : '#484f58', fontWeight: enabled ? 700 : 400, flex: 1 }}>{label}</span>
