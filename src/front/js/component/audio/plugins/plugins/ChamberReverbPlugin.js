...
export const createChamberReverbPlugin = (context, p = {}) => {
  ...
  const sr=context.sampleRate, decay=p.decay??1.2, len=Math.floor(sr*decay);
  const ir=context.createBuffer(2,len,sr);
  for(let ch=0;ch<2;ch++){ const d=ir.getChannelData(ch); for(let i=0;i<len;i++){ const t=i/sr; d[i]=(Math.random()*2-1)*(t<0.01?t/0.01:Math.exp(-(t-0.01)/(decay*0.5))); } }
  convolver.buffer=ir;
