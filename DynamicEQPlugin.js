// =============================================================================
// DynamicEQPlugin.js — StreamPireX Audio Plugin
// =============================================================================
...
// ─────────────────────────────────────────────────────────────────────────────
// 2. LinearPhaseEQPlugin.js — Zero phase EQ using FFT convolution
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// 3. MidSideEQPlugin.js — Separate EQ for mid and side channels
// ─────────────────────────────────────────────────────────────────────────────
export const createMidSideEQPlugin = (context, p = {}) => {
  const input   = context.createGain();
  const output  = context.createGain();
  ...
  const sideEQ = context.createBiquadFilter();
  midEQ.type   = 'peaking'; midEQ.frequency.value = p.midFreq ?? 500; midEQ.gain.value = p.midGain ?? 0;
  sideEQ.type  = 'peaking'; sideEQ.frequency.value = p.sideFreq ?? 3000; sideEQ.gain.value = p.sideGain ?? 0;

  input.connect(splitter);
  splitter.connect(midEQ, 0);
  splitter.connect(sideEQ, 1);
  ...
  return {
    inputNode: input, node: input,
    setParam(k, v) {
      if (k === 'midFreq')  midEQ.frequency.setTargetAtTime(v, 0, 0.01);
      if (k === 'midGain')  midEQ.gain.setTargetAtTime(v, 0, 0.01);
      if (k === 'sideFreq') sideEQ.frequency.setTargetAtTime(v, 0, 0.01);
      if (k === 'sideGain') sideEQ.gain.setTargetAtTime(v, 0, 0.01);
    },
    getState: () => ({ midFreq: midEQ.frequency.value, midGain: midEQ.gain.value, sideFreq: sideEQ.frequency.value, sideGain: sideEQ.gain.value }),
    connect: d => output.connect(d),
    disconnect: () => output.disconnect(),
  };
};
...
// ─────────────────────────────────────────────────────────────────────────────
// 4. AirEQPlugin.js — High frequency presence and air boost
// ─────────────────────────────────────────────────────────────────────────────
